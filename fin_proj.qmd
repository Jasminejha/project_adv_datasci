...
The best RMSE is x. This may be a bit higher than expected, but a good metric to have in order to compare it with the elastic net model.
...
knitr::kable(select(model_predictions, wldayr, starts_with(".pred")))
```
...
The most important variable in the model is bedayr, which is the number of days in the past year that a person was in bed for more than half the day. This, of course, makes sense since people who have to work in person cannot go to work if they are bedridden. Another important variable...
