...
#### 3.1.2 Create Candidate Model: Random Forests
...
#### 3.1.3 Create Candidate Mode: Elastic Net Regression
...
We used this model as:

-   It can work with datasets with a large number of variables

-   It can generate stable esitmates for data with multicollinearity

-   It can perform feature selection, but in a less dramatic fashion than LASSO regression
...
Cleaning data for microsimulation
...
As in the counter-factual, we estimate mean reimbursement and number eligible using survey weights (PERWEIGHT), and visualize results by income level.
...
Lower income group still has the high average reimbursement but it is lower than the counter-factual model. This adjusted policy model targets reimbursements towards lower-income individuals by incurring lower total expenditure by the government.
...
Both models provide insights into the distributional effects of the policy. While the counter-factual shows the maximum reach of benefits, the adjusted policy model reveals how narrowing eligibility shifts support toward the most disadvantaged but reduces total expenditure.
...
General things to include in the conclusion: - Implications for policymakers when deciding which predictive model to implement

-   Implications for policymakers when deciding which version of the policy to implement (cost, efficiency vs. equity, etc.)

Things we would like to do in the future if more time/processing power:

-   Include more variables in the model and try additional types of models

-   Incorporate additional datasets into the microsimulation that have more granular geographic information to explore how state policies could affect federal policy implementation
